...
const createMarker = ({ position, previousPosition, ...options }, ctx) => {
  const bearingAngle = options.customAngle||options.customAngle===0?options.customAngle:computeBearing(previousPosition, position)
...
const updateMarker = (marker, props, prevProps) => {
  ...
  // if (previousPosition?.[0] !== position[0] && previousPosition?.[1] !== position[1]) {
    ...
    const bearingAngle = props.customAngle===0||props.customAngle?props.customAngle:computeBearing(previousPosition, position)
